...
function loadProgramTexture(gl, program, image, texUnit, name) {
  gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1);//gl.UNPACK_FLIP_Y_WEBGL：Y轴方向翻转
  ...
  gl.uniform1i(uniform, texUnit);//纹理单元传递给着色器中的取样器变量

  gl.bindTexture(gl.TEXTURE_2D, null); // Unbind texture
}

function initProgramTextures(gl, program, files, names) {
  const num = files.length;
  for (var i = 0; i < num; i++) {
    const ii = i;
    const image = new Image();
    image.onload = function () {
      loadProgramTexture(gl, program, image, ii, names[ii]);
    };
    image.src = files[ii];
  }
}